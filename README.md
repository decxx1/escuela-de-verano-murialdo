...
El build necesita un CMS accesible y publicaciones vigentes. Si falla la API, la compilación falla explícitamente: no genera silenciosamente un sitio incompleto. Contacto y ajustes se leen por su slug `default`; los demás servicios consultan únicamente su tipo de contenido. Noticias y galerías usan `sort=recent`, que conserva el orden editorial y desempata por fecha de creación descendente e ID ascendente antes de paginar. Las lecturas completas recorren lotes de hasta 100 entradas, sin depender del límite de 5000 del snapshot.

El cliente comparte llamadas simultáneas y cachea cada consulta durante el build. Noticias y álbumes se pasan desde `getStaticPaths` a sus páginas para evitar consultas adicionales; las consultas individuales de los servicios usan el endpoint por slug. Los datos adaptados también se reutilizan entre el listado y las rutas. En desarrollo las consultas se renuevan después de tres segundos al navegar; los cambios se muestran sólo después de publicar en el CMS. Equipo y precios no se consultan cuando sus ajustes los ocultan. La paginación visual de los listados y fotos sigue siendo local sobre el HTML estático generado: no requiere consultar el CMS desde el navegador.
